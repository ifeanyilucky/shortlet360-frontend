 ...
 * Generates XML sitemap for the Shortlet360 frontend application
 */
export class SitemapGenerator {
  ...
  addStaticRoutes() {
    this.config.staticRoutes.forEach(route => {
      this.addUrl({
        ...
        lastmod: this.config.settings.includeLastmod ? new Date().toISOString() : null,
      });
    });
  }
  ...
  async addDynamicRoutes() {
    for (const [routeType, routeConfig] of Object.entries(this.config.dynamicRoutes)) {
      try {
        await this.fetchAndAddDynamicRoute(routeType, routeConfig);
      } catch (error) {
        console.warn(`Failed to fetch dynamic routes for ${routeType}:`, error.message);
      ...
      if (Array.isArray(data)) {
        data.forEach(item => {
          ...
          this.addUrl({
            ...
            changefreq: routeConfig.changefreq || this.config.defaults.changefreq,
  ...
  async fetchDynamicData(endpoint) {
    try {
      ...
      if (endpoint.includes('blogs')) {
        return [
          { slug: 'welcome-to-shortlet360', updatedAt: '2024-01-15T10:00:00Z' },
          { slug: 'property-management-tips', updatedAt: '2024-01-10T15:30:00Z' },
  ...
  getLastModified(item) {
    ...
    const lastModFields = ['updatedAt', 'updated_at', 'lastModified', 'last_modified', 'modifiedAt'];
