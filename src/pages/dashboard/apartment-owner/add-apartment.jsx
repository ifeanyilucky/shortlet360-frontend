...
export default function AddApartment() {
  ...
  const { addProperty, isLoading, error } = propertyStore();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.property_images.forEach((image) => {
        imageFormData.append("images", image); // 'images' must match the field name expected by the backend
      });

      // Upload all images to Cloudinary
      const response = await uploadService.uploadImages(imageFormData);
      const uploadedUrls = response.urls; // Assuming the backend returns { urls: [...] }
