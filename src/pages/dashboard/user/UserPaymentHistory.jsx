...
import { FaEye, FaFilter, FaCreditCard, FaHistory } from "react-icons/fa";
...
export default function UserPaymentHistory() {
  ...
  const [renewingRent, setRenewingRent] = useState(false);
  const { user } = useAuth();
  ...
  // Check if rent is due for renewal
  const isRentDue = (tenant) => {
    if (!tenant || tenant.lease_status !== "active") return false;

    const today = new Date();
    const leaseEndDate = new Date(tenant.lease_end_date);

    // Check if lease ends within 30 days
    const thirtyDaysFromNow = addMonths(today, 1);
    return (
      isAfter(leaseEndDate, today) && isBefore(leaseEndDate, thirtyDaysFromNow)
    );
  };

  // Check if rent is overdue
  const isRentOverdue = (tenant) => {
    if (!tenant) return false;

    const today = new Date();
    const leaseEndDate = new Date(tenant.lease_end_date);

    return isAfter(today, leaseEndDate);
  };

  // Handle rent renewal
  const handleRenewRent = async (tenant) => {
    try {
      setRenewingRent(true);

      // Navigate to property detail page for renewal
      navigate(`/property/${tenant.property_id._id}`, {
        state: {
          isRenewal: true,
          currentTenant: tenant,
          renewalType: isRentOverdue(tenant) ? "overdue" : "due",
        },
      });
    } catch (error) {
      console.error("Error initiating rent renewal:", error);
      toast.error("Failed to initiate rent renewal. Please try again.");
    } finally {
      setRenewingRent(false);
    }
  };
  ...
  const getRentStatus = (tenant) => {
    if (isRentOverdue(tenant)) {
      return {
        status: "Overdue",
        color: "bg-red-100 text-red-800",
        icon: "⚠️",
      };
    } else if (isRentDue(tenant)) {
      return {
        status: "Due Soon",
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "status",
      render: (_, row) => {
        const rentStatus = getRentStatus(row);
        return (
          <div className="space-y-1">
            ...
            <div>
              <span
                className={`px-2 py-1 rounded-full text-sm ${rentStatus.color}`}
              >
                {rentStatus.icon} {rentStatus.status}
              </span>
            </div>
          </div>
        );
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Track your rent payments and manage renewals
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-8">
          <div className="text-center">
            ...
            <p className="text-gray-400 text-sm mb-4">
              You haven't made any rent payments yet. Start exploring to find
              your perfect home.
            </p>
            <button
              onClick={() => navigate("/")}
